...
fit_loess <- function(bullet, groove, span = 0.75) {
  ...
  p2 <- ggplot2::ggplot(ggplot2::aes(x=y, y=resid), data=bullet_filter) + 
    #     geom_polygon(aes(x=x,y=y), fill="#0066cc", data=poly) +
    #      geom_line(size=0.1) +
    ggplot2::geom_line()+ 
    ggplot2::theme_bw() 
  
  p1 <- ggplot2::qplot(data = bullet_filter, y, value) +
    ggplot2::theme_bw() +
    ggplot2::geom_smooth()
...
#' @export
bulletSmooth <- function(data, span = 0.03, limits = c(-5,5), id="name") {
  bullet <- NULL
  y <- NULL
  myspan <- NULL
